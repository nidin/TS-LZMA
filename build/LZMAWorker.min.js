...
8*d}this.decoder.markerIsMandatory=!f;this.decoder.rangeDec.inStream=a;this.decoder.create();a=this.decoder.decode(f,k);if(a==b.LZMA_RES_ERROR)throw"LZMA decoding error";if(a!=b.LZMA_RES_FINISHED_WITHOUT_MARKER)if(a==b.LZMA_RES_FINISHED_WITH_MARKER){if(f){if(this.decoder.outWindow.out_pos!=k)throw"Finished with end marker before than specified size";console.log("Warning: ")}}else throw"Internal Error";console.log("\n");this.decoder.rangeDec.corrupted&&console.log("Warning: LZMA stream is corrupted");
